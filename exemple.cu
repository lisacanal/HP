...
void apply_bilateral_filter(unsigned char *input, unsigned char *output, int width, int height, int channels, int diameter, float sigma_color, float sigma_space) {
    unsigned char *d_src, *d_dst;
    size_t image_size = width * height * channels;

    cudaMalloc(&d_src, image_size);
    cudaMalloc(&d_dst, image_size);
    cudaMemcpy(d_src, input, image_size, cudaMemcpyHostToDevice);

    dim3 blockSize(16, 16);
    dim3 gridSize(32, 32);  // peut être ajusté dynamiquement

    bilateral_filter_cuda<<<gridSize, blockSize>>>(d_src, d_dst, width, height, channels, diameter, sigma_color, sigma_space);
    cudaDeviceSynchronize();
    ...
    cudaMemcpy(output, d_dst, image_size, cudaMemcpyDeviceToHost);

    cudaFree(d_src);
    cudaFree(d_dst);
}

void cleanup(unsigned char *original, unsigned char *filtered) {
...
int main(int argc, char *argv[]) {
    ...
    int width, height, channels;
    unsigned char *input_image = load_image(argv[1], &width, &height, &channels);
    unsigned char *output_image = (unsigned char *)malloc(width * height * channels);

    apply_bilateral_filter(input_image, output_image, width, height, channels, 5, 15.0f, 5.0f);
    save_image(argv[2], output_image, width, height, channels);

    cleanup(input_image, output_image);

    printf("CUDA bilateral filter complete.\n");
    return 0;
}
